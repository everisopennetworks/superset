...
import { DataColumnMeta, ActionTableChartTransformedProps } from './types';
import DataTable, {
  DataTableProps,
  SearchInputProps,
...
function ModalCreateTicket({ show = false, onClose, alarms }) {
  ...
  const handleOk = useCallback((values: any) => {
    ...
    //onClose(true);
    console.log(values);
    const response = ticketManager.createTicket({ ...values.ticket, alarms });
      ...
      >
        ...
        >
          <Select>
            ...
            <Select.Option value={2}>Mid</Select.Option>
          ...
          <List
            ...
            renderItem={item => (
              <List.Item>
                <Typography.Text
                  mark
                >{`${item.ALARMEDOBJECTNAME} ${item.ALARMTYPEID} ${item.SPECIFICPROBLEM}`}</Typography.Text>{' '}
              </List.Item>
            )}
          />
        </Form.Item>
...
export default function ActionTableChart<D extends DataRecord = DataRecord>(
  props: ActionTableChartTransformedProps<D> & {
  ...
  const handleOnItemSelect = (alarm: any, checked: boolean) => {
    setSelectedAlarms({
      ...selectedAlarms,
  ...
  const getColumnConfigs = useCallback(
    (column: DataColumnMeta, i: number) => {
      ...
      return {
        ...
        Cell: (data: any): ColumnWithLooseAccessor<D> => {
          ...
          if (key === 'item_select') {
            return (
              <td {...cellProps}>
                ...
                <Checkbox
                  onChange={e =>
                    handleOnItemSelect(row.original, e.target.checked)
                  }
  ...
  const columns = useMemo(() => columnsMeta.map(getColumnConfigs), [
    columnsMeta,
    getColumnConfigs,
  ]);
    ...
    <Styles>
      ...
      >
        <Button
          ...
          type="primary"
          disabled={
            Object.values(selectedAlarms).filter(a => a !== null).length === 0
          }
        >
          Create Ticket
        </Button>
