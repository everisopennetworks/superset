...
import { extractTimegrain, GenericDataType, getMetricLabel, getNumberFormatter, getTimeFormatter, getTimeFormatterForGranularity, NumberFormats, QueryMode, smartDateFormatter, TimeFormats } from '@superset-ui/core';
...
const processColumns = memoizeOne(function processColumns(props) {
  ...
  const rawPercentMetrics = (percentMetrics_ != null ? percentMetrics_ : []).map(getMetricLabel); // column names for percent metrics always starts with a '%' sign.
...
const transformProps = chartProps => {
  ...
  const totals = showTotals && queryMode === QueryMode.aggregate ? (_totalQuery = totalQuery) == null ? void 0 : _totalQuery.data[0] : undefined;
  const columnColorFormatters = (_getColorFormatters = getColorFormatters(conditionalFormatting, data)) != null ? _getColorFormatters : [];
  return {
    height,
    width,
