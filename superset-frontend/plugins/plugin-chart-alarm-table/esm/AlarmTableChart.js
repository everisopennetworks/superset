...
import { DTTM_ALIAS, ensureIsArray, GenericDataType, getTimeFormatterForGranularity, t, tn } from '@superset-ui/core';
import { Button, Checkbox, Input, Form, Modal, Select, List, Typography } from 'antd';
...
function ModalCreateTicket({
  ...
  const handleOk = useCallback(values => {
    setLoading(true);
    ticketManager.createTicket({ ...values.ticket,
      alarms
    });
    form.resetFields();
    onClose(true);
  ...
  return ___EmotionJSX(Modal, {
  ...
  }, ___EmotionJSX(Input, null)), ___EmotionJSX(Form.Item, {
    name: ['ticket', 'impact'],
    label: "Impact",
    rules: [{
      required: true
    }]
  }, ___EmotionJSX(Select, null, ___EmotionJSX(Select.Option, {
    value: 1
  }, "High"), ___EmotionJSX(Select.Option, {
    value: 2
  }, "Mid"), ___EmotionJSX(Select.Option, {
    value: 3
  }, "Low"))), ___EmotionJSX(Form.Item, {
    name: ['ticket', 'urgency'],
    label: "Urgency",
    rules: [{
      required: true
    ...
    value: 3
  }, "Low"))), ___EmotionJSX(Form.Item, null, ___EmotionJSX(List, {
    header: ___EmotionJSX("div", null, "Description:"),
    bordered: true,
    dataSource: alarms,
    renderItem: item => ___EmotionJSX(List.Item, {
      key: item.ALARMID
    }, ___EmotionJSX(Typography.Text, {
      mark: true
    }, `${item.ALARMEDOBJECTNAME} ${item.ALARMTYPEID} ${item.SPECIFICPROBLEM}`))
  }))));
}

export default function AlarmTableChart(props) {
  const {
    timeGrain,
    height,
    width,
  ...
  const getColumnConfigs = useCallback((column, i) => {
    ...
    return {
      ...
      Cell: ({
        ...
        if (key === 'item_select') {
          return ___EmotionJSX("td", cellProps, ___EmotionJSX(Checkbox, {
  ...
  const handleOnCreateTicked = useCallback(() => {
    setShowModal(true);
  }, [selectedAlarms]);
  const handleOnClose = useCallback(success => {
    if (success) {
      setSelectedAlarms([]);
    }
  ...
  return ___EmotionJSX(Styles, null, ___EmotionJSX("div", {
    style: {
      display: 'flex',
      flexDirection: 'row',
      justifyContent: 'end',
  ...
  }, "Create Ticket")), ___EmotionJSX(DataTable, {
    columns: columns,
    data: data,
    rowCount: rowCount,
    ...
    ,
    sticky: sticky
  }), showModal && ___EmotionJSX(ModalCreateTicket, {
